<?php /*a:1:{s:63:"C:\DATA\MyMotion\think-eflow\view\admin\wf\instance\detail.html";i:1740721614;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<script>
    ...
    const SELECT_API = "/admin/wf.instance/show" + location.search;
    ...
    layui.use(['form', 'jquery', 'formDesigner', 'layer', 'upload', 'table'], function () {
        ...
        let data = [];//表单
        let form_data = {};//表单数据
        let process_instance_id = layui.url().search[PRIMARY_KEY];

        //如果id没有传关闭当前页面
        if (process_instance_id == undefined || process_instance_id == '') {
        ...
        layui.$.ajax({
            ...
            success: function (ret) {
                if (ret.code == 0) {
                    ...
                    form_data = filterAndRemovePrefix(row.ext ?? {}, 'f_');//筛选出表单值
        ...
        $('.execute').on('click', function () {
            layer.open({
                ...
                shadeClose: true,
                area: ["850px", "450px"],
                content: HANDLE_APPROVE_URL + "?" + PRIMARY_KEY + "=" + process_task_id
            });
        });
        ...
        function tableRender(process_instance_id) {
            table.render({
                elem: "#data-table",
