<?php /*a:1:{s:72:"C:\DATA\MyMotion\think-eflow\view\admin\wf\common\design\panel\task.html";i:1740701113;}*/ ?>
<!DOCTYPE html>
<html>
    ...
    <div class="layui-tab-content">
        ...
        <div class="layui-tab-item">
            ...
            <div class="main-container">
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">表单参与者</label>
                    <div class="layui-input-block">
                        <input type="text" name="assignee_form_key" class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">用户组标识</label>
                    <div class="layui-input-block">
                        <div id="group_key"></div>
                    </div>
                </div>
                <!--                <div class="layui-form-item">-->
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" lay-verify="" placeholder="60s/1m/1h/1d 格式" autocomplete="off" name="reminder_time" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">重复间隔：</label>
                    <div class="layui-input-block">
                        <input type="text" lay-verify="" placeholder="60s/1m/1h/1d 格式" autocomplete="off" name="reminder_repeat" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">期待完成时间：</label>
                    <div class="layui-input-block">
                        <input type="text" lay-verify="" placeholder="60s/1m/1h/1d 格式" autocomplete="off" name="expire_time" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">回调处理：</label>
                    <div class="layui-input-block">
                        <input type="text" lay-verify="" placeholder="" autocomplete="off" name="callback" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">自动完成：</label>
                    <div class="layui-input-block">
                        <select name="auto_execute" lay-filter="perform_type">
...
<script>
    ...
    const GROUP_SELECT_API = "/admin/wf.designer/group";//用户组
    const ASSESSING_HANDLER_SELECT_API = "/admin/wf.designer/assigneeHandler";//用户参与类

    window.child = function (obj) {
        layui.use(['jquery', 'form', 'colorpicker', 'dropdown', 'xmSelect'], function () {
            ...
            let group = xmSelect.render({
                ...
                on: function (data) {
                    ...
                    $('input[name="group_key"]').val(stringArr.join(','))
                    dataProcessing();
                }
            });
            ...
            let propertyNames = [
                ...
                {id: "ext_user_key", title: '用户标识'},
                {id: "ext_group_key", title: '用户组标识'},
            ...
            function renderData(data, propertyNames) {
                let countersignKey = ['countersign_type'];
                let countersignAllKey = ['countersign_type', 'countersign_completion_condition'];
                ...
                for (let i = 0; i < propertyNames.length; i++) {
                    ...
                    if (data.hasOwnProperty(key)) {
                        if (countersignAllKey.includes(key)) {
                            // 如果不是会签模式跳过自定义会签属性
                            if (data['perform_type'] !== 'ALL') {
                                continue; // 跳过当前循环
                            }
                        }
                        //扩展属性没有值跳过
                        ...
                        let html = '<input type="text" name="' + key + '" value="' + data[key] + '" lay-verify="required" placeholder="请输入属性值" autocomplete="off" class="layui-input">';
                        if (countersignKey.includes(key)) {
                            html = '<select name="' + key + '" lay-filter="' + key + '">' +
                        ...
                        // 创建自定义表单项
                        let propertyItem = '<div class="custom-form-item">' +
                            '<label class="layui-form-label">' + propertyName.title + '</label>' +
                            '<div class="custom-form-content">' +
            ...
            layui.$(document).ready(function () {
                ...
                staff(newObj);
                organize(newObj);
                renderData(newObj.field, propertyNames);//渲染扩展属性
            })
            ...
            function staff(newObj) {
                if (newObj.assignee === undefined || newObj.assignee === '') {
                    return;
                }
                $.ajax({
                    url: ASSIGNEE_SELECT_API,
                    type: 'GET',
                    dataType: 'json',
                    async: false,//避免弹窗关掉还没赋值就更新了数据导致参与人丢失
                    data: {id: newObj.assignee, page: 1, limit: 999},
                    success: function (ret) {
                        let data = ret.data['items'] != undefined ? ret.data['items'] : []
             ...
             * @param newObj
             */
            function organize(newObj) {
                if (newObj.group_key === undefined || newObj.group_key === '') {
                    return;
                }
                $.ajax({
                    url: GROUP_SELECT_API,
                    type: 'GET',
                    dataType: 'json',
                    async: false,//避免弹窗关掉还没赋值就更新了数据导致数据丢失
                    data: {id: newObj.group_key, page: 1, limit: 999},
                    success: function (ret) {
                        let data = ret.data['items'] != undefined ? ret.data['items'] : []
                        group.setValue(data);
                        let stringArr = [];
            ...
            function assessingHandler(defaultValue = '') {
                $.ajax({
                    url: ASSESSING_HANDLER_SELECT_API,
                    type: 'GET',
                    dataType: 'json',
                    success: function (ret) {
                        let data = ret.data;
                        // 渲染下拉框
                        let selectHtml = '<option value="">请选择参与处理类</option>';
                        for (let i = 0; i < data.length; i++) {
