...
class Index extends Base
{
    ...
    public function index()
    {
        if (Request::isAjax()) {
            ...
            $this->jsonApi('ok', 200, $data);
        }
        return $this->fetch();
    }

    private function createTables($db, $pk): bool|string
    {
        $sql       = str_replace(['{{$pk}}'], [$pk], file_get_contents(app_path() . 'data' . DS . 'data.sql'));
        $sql_array = preg_split("/;[\r\n]+/", $sql);
        foreach ($sql_array as $k => $v) {
            if (!empty($v)) {
                if (str_starts_with($v, 'CREATE TABLE')) {
