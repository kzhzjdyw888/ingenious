...
abstract class BaseController
{
    ...
    // 获取系统参数
    protected function getSystem(): array
    {
        return [
            'os'      => PHP_OS,
            ...
            'ver'     => config('app.app_versions'),
        ];
    }

    protected function getJson($json = []): ?Json
    {
        if ('json' == strtolower($this->getResponseType())) {
            return $this->json($json['msg'] ?? '操作成功', $json['code'] ?? 200, $json['data'] ?? [], $json['extend'] ?? []);
        }
        return $this->json('操作失败', 400);
    }

    //页面分配变量
    protected function assign($key, $value): \think\View
    {
        return View::assign($key, $value);
    }

    //页面渲染
    protected function fetch($template = '', $data = []): string
    {
