...
abstract class BaseService
{
     ...
     * @return bool
     */
    public function batchDelete(array $ids, ?string $key = null): bool
    {
        return $this->getModel()->whereIn(is_null($key) ? $this->getPk() : $key, $ids)->delete();
    }
     ...
     * @return mixed
     */
    public function updated(string|int|array $id, array $data, ?string $key = null): mixed
    {
        if (is_array($id)) {
            $where = $id;
        } else {
     ...
     * @return mixed
     */
    public function save(array $param): mixed
    {
        return $this->getModel()::create($param);
    }
