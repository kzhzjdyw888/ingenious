...
class SystemRoleServices extends BaseServices
{
    ...
    public function verifyAuth(Request $request): bool
    {
        ...
        $rule   = $request->path();
        if ($isPath !== false) {
            $rule = substr($request->path(), $isPath + strlen($request->app) + 1);
            $rule = ltrim($rule, '/');
        }
        ...
        $allAuth = $this->cacheDriver()->remember('all_auth', function () {
            ...
            $menusService = Container::make(SystemMenusServices::class);
    ...
    {
        $systemRoleMenuService = Container::make(SystemRoleMenuServices::class);
