...
class SystemAdminServices extends BaseServices
{
    ...
    {
        $this->dao = Container::make(SystemAdminDao::class);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException|\app\exception\AdminException
     */
    public function find(string $id): array
    {
        ...
        // 解析 SystemAdminDeptServices 类的实例
    ...
    public function verifyLogin(string $account, string $password, $isPwd = true): bool|array|Model
    {
        ...
        // 获取客户端 IP 地址
        $clientIp = request()->header('x-real-ip') ?? request()->header('x-forwarded-for') ?? $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';

        $adminInfo->last_time = time();
        $adminInfo->last_ip   = $clientIp;
    ...
    public function login(string $account, string $password, string $type, string $key = '', $isPwd = true)
    {
        ...
        //获取队列状态
        $remind = Config('app.console_remind', false);
...
//        $service = Container::make(SystemRoleServices::class);
