...
class SystemRoleDao extends BaseDao
{
     ...
     */
    public function getRouleList(array $where, int $page, int $limit, string $sort = 'sort asc')
    {
        return $this->search($where)->page($page, $limit)->order($sort)->select()->toArray();
    }

    /**
     * 指定条件获取某些角色的名称以数组形式返回
     *
     * @param array  $where
     * @param string $field
     * @param string $key
