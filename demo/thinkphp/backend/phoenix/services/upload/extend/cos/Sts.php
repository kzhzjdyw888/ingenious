...
// 临时密钥计算样例
    ...
    // obj 转 query string
    function json2str($obj, $notEncode = false): string
    {
        ksort($obj);
        $arr = array();
        if (!is_array($obj)) {
    ...
    // 计算临时密钥用的签名
    function getSignature($opt, $key, $method, $config): string
    {
    ...
    function backwardCompat($result): array
    {
        if (!is_array($result)) {
    ...
    // 获取临时密钥
    function getTempKeys($config): array
    {
        $result = null;
        try {
    ...
    //申请角色授权
    function getRoleCredential($config): array
    {
        $result = null;
        try {
            if (array_key_exists('policy', $config)) {
