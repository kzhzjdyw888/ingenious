...
class SystemConfigServices extends BaseServices
{
    ...
    public function __construct()
    {
        ...
        $this->builder = Container::make(FormBuilder::class);
    }
     ...
     * @date   2023/04/12
     */
    public function getSonConfig(): array
    {
        $sonConfig  = [];
     ...
     */
    public function getConfigList(array $where): array
    {
     ...
     * @throws \think\exception\DbException|\think\db\exception\DbException
     */
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getReadList($tabId): array
    {
     ...
     * @return array
     */
    public function createTextForm(string $type, array $data): array
    {
        $formbuider = [];
        switch ($type) {
            case 'number':
     ...
     * @return mixed
     */
    public function createTextareaForm(array $data): mixed
    {
     ...
     * @return array
     */
    public function createRadioForm(array $data, $control = [], $control_two = []): array
    {
        $formbuider    = [];
     ...
     * @return array
     */
    public function createUploadForm(int $type, array $data): array
    {
        $formbuider = [];
        switch ($type) {
            case 1:
     ...
     */
    public function createCheckboxForm(array $data): array
    {
        $formbuider    = [];
        $data['value'] = json_decode($data['value'], true) ?: [];
        $parameter     = explode("\n", $data['parameter']);
     ...
     * @throws \FormBuilder\Exception\FormBuilderException
     */
    public function createSelectForm(array $data): array
    {
        $formbuider    = [];
        $data['value'] = json_decode($data['value'], true) ?: [];
        $parameter     = explode("\n", $data['parameter']);
     ...
     * @return mixed
     */
    public function createColorForm(array $data): mixed
    {
        $data['value'] = json_decode($data['value'], true) ?: '';
        $formbuider[]  = $this->builder->color($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
            'type'     => 'div',
    ...
    public function bindBuilderData($data, $relatedRule): bool|array
    {
     ...
     * @throws \FormBuilder\Exception\FormBuilderException
     */
    public function formTypeShine($data, $control = false, $controle_two = []): array
    {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function createConfigForm(int $tabId, array $relatedRule): bool|array
    {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function createForm(array $list): array
    {
        if (!$list) return [];
