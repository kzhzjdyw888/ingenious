...
class SystemCrontabServices extends BaseServices
{
     ...
     */
    public function getTimerList(array $where = []): array
    {
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->selectList($where, '*', $page, $limit, 'create_time desc');
     ...
     * @return bool
     */
    public function saveTimer(array $data = []): bool
    {
        if (!$data['id'] && $this->dao->getCount(['mark' => $data['mark'], 'delete_time' => null])) {
     ...
     * @return bool
     */
    public function delTimer($id): bool
    {
        $res = $this->dao->destroy($id);
    ...
    public function crontabRun()
    {
        ...
        $list = $this->dao->selectList(['is_open' => 1, 'delete_time' => null])->toArray();
        foreach ($list as $item) {
